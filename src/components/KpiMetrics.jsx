    ...
    <section className="kpi-grid">
      ...
      <div className={`kpi-card ${diagnosis.severity === 'critical' ? 'red-glow' : diagnosis.severity === 'warning' ? 'amber-glow' : 'emerald-glow'}`}>
        <div className="kpi-top">
          ...
          <span className={`kpi-badge ${diagnosis.severity === 'critical' ? 'red' : (diagnosis.confidence >= 90 ? 'emerald' : 'amber')}`}>
            {diagnosis.confidence !== undefined ? `PIML AI: ${diagnosis.confidence.toFixed(1)}%` : diagnosis.badge}
          </span>
        </div>
